...
# Filter queries where $1 appears more than once, OR where $1 is used with ANY/IN/COALESCE/CASE/OR
suspicious = []
for fpath, line, sql in queries:
    ...
    elif '$1' in sql and any(k in sql.upper() for k in ['COALESCE', 'ANY(', 'IN (']):
        suspicious.append((fpath, line, sql, "$1 in expression/function"))
